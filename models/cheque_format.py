...
class Cheque_Format(models.Model):
    _name = 'cheque.format'
    _description = "The company cheques will have this attributes"
    ...
    def check_for_format(self):
        if self.set_default:
            check_other_default = self.search([('id','!=',self.id),('set_default','=',True)])
            # checks for other record which are not the current and if it finds another default raise and erro
            if check_other_default:
                print("there are other default template")
                raise ValidationError("Select only one default format")
        # rec = self.env['cheque.format'].search([('set_default','=',True)])
        # print("on change")
        #
        # if len(rec) > 1:
        #     raise ValidationError("Select Only One Cheque Format")